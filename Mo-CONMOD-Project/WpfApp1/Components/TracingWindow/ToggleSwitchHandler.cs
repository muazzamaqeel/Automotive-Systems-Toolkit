...
namespace WpfApp1.Components.TracingWindow
{
    public class ToggleSwitchHandler
    {
        [DllImport("kernel32.dll", SetLastError = true)]
        static extern nint CreateMutex(nint lpMutexAttributes, bool bInitialOwner, string lpName);

        [DllImport("kernel32.dll", SetLastError = true)]
        static extern bool ReleaseMutex(nint hMutex);

        [DllImport("kernel32.dll", SetLastError = true)]
        static extern nint GetLastError();

        [DllImport("USBaccessX64.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern nint FCWInitObject();

        [DllImport("USBaccessX64.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern int FCWOpenCleware(nint cw);

        [DllImport("USBaccessX64.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern int FCWGetSwitch(nint cw, int switchIndex, int switchId);

        [DllImport("USBaccessX64.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern int FCWSetSwitch(nint cw, int switchIndex, int switchId, int state);

        [DllImport("USBaccessX64.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern void FCWCloseCleware(nint cw);

        [DllImport("USBaccessX64.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern void FCWUnInitObject(nint cw);

        private nint cwPointer;
        ...
        private void InitializeMutex()
        {
            ...
            if (!isNewInstance)
            {
                MessageBox.Show("Another instance is already running.", "Instance Error", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        private void InitializeCleware()
        {
            ...
            if (devCnt <= 0)
            {
                MessageBox.Show("No Cleware devices found. Please connect the device and try again.", "Device Error", MessageBoxButton.OK, MessageBoxImage.Error);
                WpfApplication.Current.Shutdown();
                return;
            }
            ...
            if (currentState == -1)
            {
                MessageBox.Show("Failed to retrieve the current switch state.", "Switch Error", MessageBoxButton.OK, MessageBoxImage.Error);
                WpfApplication.Current.Shutdown();
                return;
            }
        ...
        public void ToggleSwitch_Checked(object sender, RoutedEventArgs e)
        {
            ...
            if (result <= 0)
            {
                MessageBox.Show("Failed to turn on the switch.", "Switch Error", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        public void ToggleSwitch_Unchecked(object sender, RoutedEventArgs e)
        {
            ...
            if (result <= 0)
            {
                MessageBox.Show("Failed to turn off the switch.", "Switch Error", MessageBoxButton.OK, MessageBoxImage.Error);
